...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['id'])) {
        ...
        $insert_query = "update artists set name = '$name', bio = '$bio', image = '$image' where id = '$id'";
        if ($conn->query($insert_query) === TRUE) {
            $response = array("status" => "success", "message" => "Cập nhật thông tin nghệ sĩ thành công");
        } else {
            $response = array("status" => "error", "message" => "Lỗi khi cập nhật nghệ sĩ: " . $conn->error);
        }
