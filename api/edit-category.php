...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['id']) && isset($data['category'])) {
        ...
        $insert_query = "update categories set category = '$category' where id = '$id'";
        if ($conn->query($insert_query) === TRUE) {
            $response = array("status" => "success", "message" => "Cập nhật danh mục thành công");
        } else {
            $response = array("status" => "error", "message" => "Lỗi khi cập nhật danh mục: " . $conn->error);
        }
