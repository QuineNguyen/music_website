...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['id'])) {
        ...
        $insert_query = "delete from artists where id = '$id' limit 1";
        if ($conn->query($insert_query) === TRUE) {
            $response = array("status" => "success", "message" => "Xóa thông tin nghệ sĩ thành công");
        } else {
            $response = array("status" => "error", "message" => "Lỗi khi xóa: " . $conn->error);
        }
