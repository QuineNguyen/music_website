...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data['username'])) {
        ...
        $insert_query = "insert into users (username, email, password, role, date) values ('$username', '$email', '$password', '$role', '$date')";
        if ($conn->query($insert_query) === TRUE) {
            $response = array("status" => "success", "message" => "Tạo tài khoản người dùng thành công");
        } else {
            $response = array("status" => "error", "message" => "Lỗi khi thêm tài khoản: " . $conn->error);
        }
